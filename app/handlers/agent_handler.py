...
try:
    # Пробуем импортировать из ip_checks.py в той же директории
    ...
    print(f"Ошибка импорта ip_checks: {e}")
    # Заглушки для случая, если модуль не найден
...
async def verify_agent_token(token):
    db = get_db()
