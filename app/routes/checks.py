...
async def get_stats_handler(request):
    ...
    try:
        ...
        return web.json_response({"error": f"Internal error: {str(e)}"}, status=500)
@checks_routes.delete('/api/check/{check_id}')
async def delete_check_handler(request):
    """Удалить проверку"""
    try:
        check_id = request.match_info['check_id']
        service = get_check_service(request.app)
        
        # Удаляем проверку из хранилища
        if check_id in service.checks_storage:
            del service.checks_storage[check_id]
            return web.json_response({"status": "deleted", "check_id": check_id})
        else:
            return web.json_response({"error": "Check not found"}, status=404)
            
    except Exception as e:
        return web.json_response({"error": f"Internal error: {str(e)}"}, status=500)

@checks_routes.post('/api/history/sync')
async def sync_history_handler(request):
    """Синхронизировать историю с клиента"""
    try:
        data = request._json_data if hasattr(request, '_json_data') else await request.json()
        # Просто подтверждаем получение, так как история хранится локально
